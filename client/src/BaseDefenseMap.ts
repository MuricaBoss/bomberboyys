...
export class BaseDefenseScene_Map extends BaseDefenseScene_Data {
  ...
  getClientUnitWaypoint(unitId: string, unit: any, now: number, unitRadius = this.localUnitBodyRadius(unit)) {
    ...
    if (needRecalc) {
      ...
      // Build 244: Smart Path Entry — Find the closest FORWARD waypoint to prevent running backwards.
      ...
      // Prefer closest forward node; only fall back to raw nearest if none found forward

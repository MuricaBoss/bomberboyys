...
const DISPLAY_BUILD_NUMBER = 20;
const MIN_CAMERA_ZOOM = 0.28;
const MAX_CAMERA_ZOOM = 2.2;
const TOUCH_GESTURE_SCROLL_BOOST = 2.35;
...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  async create() {
    ...
    this.input.on("pointermove", (pointer: Phaser.Input.Pointer) => {
      if (this.touchPinching && this.isTouchPointer(pointer)) {
  ...
  initializeWorld() {
    ...
    if (me) {
      this.cameras.main.centerOn(Number(me.x), Number(me.y));
      this.clampCameraToWorld();
  ...
  clampCameraToWorld() {
    ...
    const cameraCenterX = cam.scrollX + viewW * 0.5;
    const cameraCenterY = cam.scrollY + viewH * 0.5;
    const targetCenterX = viewW >= worldW ? centerX : Phaser.Math.Clamp(cameraCenterX, viewW * 0.5, worldW - viewW * 0.5);
    const targetCenterY = viewH >= worldH ? centerY : Phaser.Math.Clamp(cameraCenterY, viewH * 0.5, worldH - viewH * 0.5);
  ...
  updateRtsCamera(delta: number) {
    ...
    if (this.touchPinching && this.getActiveTouchPointers().length >= 2) {
      this.camVelX = 0;
      this.camVelY = 0;
      if (Math.abs(this.touchGestureScrollVX) > 0.5 || Math.abs(this.touchGestureScrollVY) > 0.5) {
        cam.scrollX += this.touchGestureScrollVX * dt;
        cam.scrollY += this.touchGestureScrollVY * dt;
        this.clampCameraToWorld();
      }
      return;
    }
    ...
    const hasTouchScrollIntent = this.touchPinching
      && this.getActiveTouchPointers().length >= 2
      && (Math.abs(this.touchGestureScrollVX) > 1 || Math.abs(this.touchGestureScrollVY) > 1);
    const targetSpeed = 560 / cam.zoom;
    const targetVX = hasTouchScrollIntent ? this.touchGestureScrollVX : intentX * targetSpeed;
    const targetVY = hasTouchScrollIntent ? this.touchGestureScrollVY : intentY * targetSpeed;
    const response = hasTouchScrollIntent ? 8 : (len > 0.0001 ? 14 : 11);
  ...
  beginTouchPinch() {
    ...
    const world = this.screenToWorldPoint(midX, midY);
  ...
  updateTouchPinch() {
    ...
    const midY = (a.y + b.y) * 0.5;
    const cam = this.cameras.main;
    const deltaPx = dist - this.touchPinchStartDist;
    const deadZonePx = 28;
    const dampFactor = 0.42;
    const effectiveDeltaPx = Math.sign(deltaPx) * Math.max(0, Math.abs(deltaPx) - deadZonePx) * dampFactor;
    const effectiveDist = Math.max(1, this.touchPinchStartDist + effectiveDeltaPx);
    const nextZoom = Phaser.Math.Clamp(this.touchPinchStartZoom * (effectiveDist / this.touchPinchStartDist), this.getMinCameraZoom(), MAX_CAMERA_ZOOM);
    const now = performance.now();
    const dtMs = Math.max(1, now - this.touchPinchLastMoveAt);
    const midDx = midX - this.touchPinchLastMidX;
    const midDy = midY - this.touchPinchLastMidY;
    const midMove = Math.hypot(midDx, midDy);
    const distDeltaSinceLast = dist - this.touchPinchLastDist;
    const zoomDominant = Math.abs(distDeltaSinceLast) > midMove * 0.9 + 1.2;
    if (midMove > 0.6) {
      const worldPerSecond = ((1000 / dtMs) / Math.max(0.001, nextZoom)) * TOUCH_GESTURE_SCROLL_BOOST;
      if (zoomDominant) {
        this.touchGestureScrollVX = 0;
        this.touchGestureScrollVY = 0;
      } else {
        this.touchGestureScrollVX = midDx * worldPerSecond;
        this.touchGestureScrollVY = midDy * worldPerSecond;
      }
      this.touchPinchLastMidX = midX;
      this.touchPinchLastMidY = midY;
      this.touchPinchLastMoveAt = now;
    } else if (Math.abs(distDeltaSinceLast) > 0.4) {
      this.touchGestureScrollVX = 0;
      this.touchGestureScrollVY = 0;
    }
    cam.scrollX += midDx / Math.max(0.001, cam.zoom);
    cam.scrollY += midDy / Math.max(0.001, cam.zoom);
    const focusWorld = this.screenToWorldPoint(midX, midY);
    cam.setZoom(nextZoom);
    cam.scrollX = focusWorld.x - midX / nextZoom;
    cam.scrollY = focusWorld.y - midY / nextZoom;
    this.syncCameraAfterZoom();
    this.layoutBaseDefenseHud();
    this.touchPinchLastDist = dist;
    if (Math.hypot(midDx, midDy) > 1.2 || Math.abs(nextZoom - this.touchPinchStartZoom) > 0.003) {
  ...
  handleMobilePointerUp(pointer: Phaser.Input.Pointer) {
    ...
    if (me?.isAlive && !me.isCoreAnchored && !this.touchMoved) {
      if (this.isClickOnOwnPlayer(world.x, world.y)) {
        const now = Date.now();
        if (now - this.lastSelfClickAt <= 320) {
          this.room.send("anchor_base");

...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  updateTouchPinch() {
    ...
    const midMovePx = Math.hypot(
      midX - (this.touchWorldFocusX - cam.scrollX) * cam.zoom,
      midY - (this.touchWorldFocusY - cam.scrollY) * cam.zoom,
    );
    cam.setZoom(nextZoom);
    cam.scrollX = this.touchWorldFocusX - midX / nextZoom;
    cam.scrollY = this.touchWorldFocusY - midY / nextZoom;
    this.clampCameraToWorld();
    cam.preRender();
    this.layoutBaseDefenseHud();
    if (midMovePx > 3 || Math.abs(nextZoom - this.touchPinchStartZoom) > 0.003) {

...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  adjustMobileZoom(delta: number) {
    ...
    cam.preRender();
    const focusX = this.cameraFocusWorldX || cam.midPoint.x;
    const focusY = this.cameraFocusWorldY || cam.midPoint.y;
    cam.setZoom(nextZoom);
    cam.preRender();
    cam.setScroll(focusX - cam.displayWidth * 0.5, focusY - cam.displayHeight * 0.5);

...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  getCameraScrollForCenterWorld(centerX: number, centerY: number) {
    ...
    const scrollX = viewW >= worldW
      ? (worldW - viewW) * 0.5
      : Phaser.Math.Clamp(centerX - viewW * 0.5, 0, worldW - viewW);
    const scrollY = viewH >= worldH
      ? (worldH - viewH) * 0.5
      : Phaser.Math.Clamp(centerY - viewH * 0.5, 0, worldH - viewH);
  ...
  adjustMobileZoom(delta: number) {
    ...
    this.setCameraCenterWorld(centerWorldX, centerWorldY);

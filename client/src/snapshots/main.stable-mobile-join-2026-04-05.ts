...
function isLikelyMobileDevice() {
  if (navigator.maxTouchPoints >= 2) return true;
...
class MenuScene extends Phaser.Scene {
  serverReady = false;
  hasStarted = false;
  selectedMode: "bomber_room" | "base_defense_room" = isLikelyMobileDevice() ? "base_defense_room" : "bomber_room";
  ...
  create() {
    const mobileDefault = isLikelyMobileDevice();
    this.selectedMode = mobileDefault ? "base_defense_room" : this.selectedMode;
    ...
    const instructionText = mobileDefault ? "Tap to Join" : "Press SPACE to Join";
    const instruction = this.add.text(this.cameras.main.centerX, this.cameras.main.centerY + 50, instructionText, { fontSize: '36px', color: '#ffffff', fontFamily: 'Arial' }).setOrigin(0.5);
    ...
    const mobileSuffix = mobileDefault ? " [mobile default]" : "";
    this.modeText = this.add.text(this.cameras.main.centerX, this.cameras.main.centerY + 145, `Mode: Bomber (1) | Base Defense (2)  [selected: ${modeLabel}]${mobileSuffix}`, { fontSize: "20px", color: "#9fe6ff", fontFamily: "Arial" }).setOrigin(0.5);
    ...
    const startSelectedMode = () => {
      ...
      if (this.selectedMode === "base_defense_room") this.scene.start("BaseDefenseScene");
...
class BaseDefenseUiScene extends Phaser.Scene {
  buttons: Array<{
  ...
  create() {
    ...
    this.input.on("pointerdown", (pointer: Phaser.Input.Pointer) => {
      ...
      for (const b of this.bottomBuildButtons) {
        ...
        world.selectedBuild = b.type;
        world.actionMode = "build";
        const reason = world.getBuildBlockedReason(b.type);
        if (reason) world.showNotice(`Cannot build: ${reason}`, "#ffb080");
        else world.showNotice(`Tap map to place ${world.buildLabel(b.type)}`, "#9fe8ff");
        return;
      }
      ...
      for (const b of this.bottomUnitButtons) {
        ...
        if (!this.pointInRect(pointer.x, pointer.y, b.rect)) continue;
        if (b.action === "soldier") {
          const reason = world.getUnitProduceBlockedReason();
          if (reason) world.showNotice(`Cannot produce: ${reason}`, "#ffb080");
          else world.room?.send("produce_unit");
        } else if (b.action === "tank") {
          const reason = world.getFactoryProduceBlockedReason("tank");
          if (reason) world.showNotice(`Cannot produce: ${reason}`, "#ffb080");
          else world.room?.send("produce_tank");
        } else {
          const reason = world.getFactoryProduceBlockedReason("harvester");
          if (reason) world.showNotice(`Cannot produce: ${reason}`, "#ffb080");
      ...
      for (const b of this.buttons) {
        ...
        else if (b.id === "full") world.toggleFullscreen();
        else if (b.id === "zoom_in") world.adjustMobileZoom(0.18);
        else if (b.id === "zoom_out") world.adjustMobileZoom(-0.18);
        else world.mobileCommandMode = b.id;
        return;
      }
    });
  }
  ...
  update() {
    ...
    const me = world.room?.state?.players?.get
      ? world.room.state.players.get(world.currentPlayerId)
      : world.room?.state?.players?.[world.currentPlayerId];
    const anchored = !!me?.isCoreAnchored || !!me?.devMode;
    const visible = world.isMobileInput || world.desktopMobileFallback;

    for (const b of this.buttons) {
      const selected = (b.id === "select" || b.id === "move" || b.id === "attack") && world.mobileCommandMode === b.id;
    ...
    if (this.bottomBg) this.bottomBg.setVisible(visible && anchored);
    if (this.bottomStatsText) {
      this.bottomStatsText.setVisible(visible && anchored);
      if (anchored && me) {
        const powerNet = Number(me.powerProduced || 0) - Number(me.powerUsed || 0);
        this.bottomStatsText.setText(
          `Credits: ${Number(me.resources || 0)} | Power: ${Number(me.powerProduced || 0)}/${Number(me.powerUsed || 0)} (${powerNet >= 0 ? "+" : ""}${powerNet})`
        );
      }
    }
    ...
    for (const b of this.bottomBuildButtons) {
      const reason = world.getBuildBlockedReason(b.type);
      const selected = world.selectedBuild === b.type && world.actionMode === "build";
      const canBuild = !reason;
      b.rect.setVisible(visible && anchored);
      b.title.setVisible(visible && anchored);
      b.meta.setVisible(visible && anchored);
      b.rect.setFillStyle(
      ...
      b.meta.setText(reason ? reason : `Cost: ${b.type === "ore_refinery" ? 55 : b.type === "solar_panel" ? 40 : b.type === "barracks" ? 80 : 130} | ${b.type === "ore_refinery" ? 5 : b.type === "solar_panel" ? 4 : 7}s`);
      b.meta.setColor(canBuild ? "#d8ecff" : "#ffb3b3");
    }

    for (const b of this.bottomUnitButtons) {
      const reason = b.action === "soldier"
        ? world.getUnitProduceBlockedReason()
        : world.getFactoryProduceBlockedReason(b.action);
      const canProduce = !reason;
      const okColor = b.action === "soldier" ? 0x364225 : 0x2b3547;
      const okStroke = b.action === "soldier" ? 0xaddb95 : 0x9cc6ff;
      b.rect.setVisible(visible && anchored);
      b.title.setVisible(visible && anchored);
      b.meta.setVisible(visible && anchored);
      b.rect.setFillStyle(canProduce ? okColor : 0x2a2a2a, 0.92);
      b.rect.setStrokeStyle(2, canProduce ? okStroke : 0x666666);
      if (reason) {
...
class GameScene extends Phaser.Scene {
  ...
  initializeWorld(state: any) {
    ...
    if (state.map && typeof state.map.onChange === "function") {
      state.map.onChange((item: number, index: number) => {
    ...
    const addPlayerEntity = (player: any, sessionId: string) => {
      ...
      console.log("PLAYER ADDED FIRED!", sessionId);
...
class BaseDefenseScene extends Phaser.Scene {
  desktopMobileFallback = isLikelyMobileDevice();
  room!: Room<any>;
  currentPlayerId = "";
  ...
  isMobileInput = !this.desktopMobileFallback && isLikelyMobileDevice();
  ...
  async create() {
    ...
    }).setPadding(8).setScrollFactor(0).setDepth(205).setVisible(this.isMobileInput);
    ...
    try {
      this.room = await client.joinOrCreate("base_defense_room");
      loading.setText("Joined Base Defense. Syncing state...");
      ...
      if ((this.isMobileInput || this.desktopMobileFallback) && !this.scene.isActive("BaseDefenseUiScene")) {
        this.scene.launch("BaseDefenseUiScene");
        this.scene.bringToTop("BaseDefenseUiScene");
      }
      this.scale.on("resize", this.handleViewportResize, this);
      this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
        this.scale.off("resize", this.handleViewportResize, this);
        if (this.scene.isActive("BaseDefenseUiScene")) this.scene.stop("BaseDefenseUiScene");
      ...
      this.room.onMessage("game_over", (data?: { winnerName?: string }) => {
        const txt = this.add.text(this.cameras.main.centerX, this.cameras.main.centerY - 100, data?.winnerName ? `${data.winnerName} wins` : "Game Over", {
          fontSize: "54px",
          color: "#ffff00",
          fontFamily: "Arial",
          fontStyle: "bold",
        }).setOrigin(0.5).setDepth(120).setScrollFactor(0);
        this.time.delayedCall(2600, () => txt.destroy());
      });
      this.room.onMessage("base_attacked", (data?: { ownerId?: string }) => {
        if (data?.ownerId === this.currentPlayerId) this.showNotice("BASE UNDER ATTACK", "#ff8c8c");
      });
      this.room.onMessage("harvester_attacked", (data?: { ownerId?: string }) => {
        if (data?.ownerId === this.currentPlayerId) this.showNotice("HARVESTER UNDER ATTACK", "#ffb080");
      });
      this.room.onMessage("unit_lost", (data?: { ownerId?: string; unitType?: string }) => {
        if (data?.ownerId !== this.currentPlayerId) return;
        const unitName = data?.unitType === "harvester" ? "Harvester" : "Soldier";
        this.showNotice(`${unitName.toUpperCase()} LOST`, "#ffd38a");
      });
      this.room.onMessage("building_lost", (data?: { ownerId?: string; type?: string }) => {
        if (data?.ownerId !== this.currentPlayerId) return;
        const t = String(data?.type || "building").replaceAll("_", " ").toUpperCase();
        this.showNotice(`${t} LOST`, "#ff9f9f");
      });
      this.room.onMessage("dev_mode", (data?: { playerId?: string; enabled?: boolean }) => {
        if (data?.playerId !== this.currentPlayerId) return;
        this.showNotice(data?.enabled ? "Developer mode ON" : "Developer mode OFF", data?.enabled ? "#9fe8ff" : "#ffcc88");
      });
      const handleShot = (data?: { fromX?: number; fromY?: number; toX?: number; toY?: number; team?: string; unitType?: string }) => {
        if (!data) return;
        ...
        const myTeam = this.room.state.players.get
          ? this.room.state.players.get(this.currentPlayerId)?.team
          : this.room.state.players?.[this.currentPlayerId]?.team;
        const friendly = !!myTeam && data.team === myTeam;
        const isTank = data.unitType === "tank";
        const color = isTank
          ? (friendly ? 0xb9ecff : 0xffb1a8)
          : (friendly ? 0xe8f8ff : 0xffd4d4);
        const width = isTank ? 2.2 : 1.3;
        const duration = isTank ? 150 : 95;
        const glowRadius = isTank ? 3.2 : 2.0;
        const line = this.add.line(0, 0, fromX, fromY, toX, toY, color, 0.9).setLineWidth(width, width).setDepth(26);
        const glow = this.add.circle(toX, toY, glowRadius, color, 0.95).setDepth(27);
        this.tankShotEffects.push({ line, glow, expiresAt: Date.now() + duration });
        // Track shot direction for barrel facing
        const shotAngle = Math.atan2(toY - fromY, toX - fromX);
        const shotDir = this.angleToDir8(shotAngle);
        // Find which unit fired from this position
        if (this.room?.state?.units?.forEach) {
          this.room.state.units.forEach((fu: any, fid: string) => {
            if (Math.hypot(Number(fu.x) - fromX, Number(fu.y) - fromY) < TILE_SIZE * 0.8) {
              this.unitLastShotDir.set(fid, { dir: shotDir, at: Date.now() });
            }
          });
        }
      };
      this.room.onMessage("unit_shot", handleShot);
      // Backward compatibility for older server packets.
      this.room.onMessage("tank_shot", handleShot);

      this.room.onError((code: number, message?: string) => {
        if (!this.hasInitialized) loading.setText(`Room error ${code}: ${message || "unknown"}`);
      });
      this.room.onLeave((code: number) => {
        if (!this.hasInitialized) loading.setText(`Disconnected before state (${code})`);
      });

      this.room.onStateChange(() => {
        if (!this.hasInitialized && this.room.state.mapWidth > 0 && this.room.state.mapHeight > 0 && this.mapItemCount(this.room.state.map) > 0) {
          this.initializeWorld();
          loading.destroy();
          preInitDebug.destroy();
        }
      });
      const tryInitializeFromCurrentState = () => {
        if (!this.hasInitialized && this.room?.state?.mapWidth > 0 && this.room?.state?.mapHeight > 0 && this.mapItemCount(this.room.state.map) > 0) {
          this.initializeWorld();
          loading.destroy();
          preInitDebug.destroy();
      ...
      if (!tryInitializeFromCurrentState()) {
        this.time.addEvent({
          delay: 250,
          repeat: 40,
          callback: () => {
            const mapWidth = Number(this.room?.state?.mapWidth || 0);
            const mapHeight = Number(this.room?.state?.mapHeight || 0);
            const mapCount = this.mapItemCount(this.room?.state?.map);
            preInitDebug.setText(
              `Scene: BaseDefense active=${this.scene.isActive()}\n`
              + `Joined: ${!!this.room} session=${this.currentPlayerId || "-"}\n`
              + `Camera: ${Math.round(this.cameras.main.width)}x${Math.round(this.cameras.main.height)} zoom=${this.cameras.main.zoom.toFixed(2)}\n`
              + `State: mapWidth=${mapWidth} mapHeight=${mapHeight} mapCount=${mapCount}\n`
              + `HasInit: ${this.hasInitialized}\n`
              + `Mode: mobile=${this.isMobileInput}`
            );
            if (tryInitializeFromCurrentState()) return;
            if (!this.hasInitialized) {
              loading.setText("Joined Base Defense. Waiting for world state...");
            }
          },
        });
      }
    } catch (err) {
      console.error("base join error", err);
      loading.setText("Base mode join failed.\nPress SPACE to return");
      preInitDebug.setText(`BaseDefense join error:\n${String((err as any)?.message || err)}`);
      this.input.keyboard!.once("keydown-SPACE", () => this.scene.start("MenuScene"));
    }

    this.selectionRectGraphics = this.add.graphics().setDepth(200).setScrollFactor(0);
    this.attackCursorGraphics = this.add.graphics().setDepth(220);

    this.input.on("pointerdown", (pointer: Phaser.Input.Pointer) => {
      if (!this.room?.state || !this.hasInitialized) return;
      if (this.isMobileInput && this.handleMobilePointerDown(pointer)) return;
    ...
    this.input.on("pointermove", (pointer: Phaser.Input.Pointer) => {
      if (this.isMobileInput && this.handleMobilePointerMove(pointer)) return;
      ...
      const p1 = this.worldToScreenPoint(this.selectionStart.x, this.selectionStart.y);
      const p2 = this.worldToScreenPoint(world.x, world.y);
      const x1 = p1.x;
      const y1 = p1.y;
      const x2 = p2.x;
      const y2 = p2.y;
      ...
      this.selectionRectGraphics.lineStyle(2, 0x00ffcc, 0.95);
      this.selectionRectGraphics.fillStyle(0x00ffcc, 0.15);
      this.selectionRectGraphics.fillRect(rx, ry, rw, rh);
      this.selectionRectGraphics.strokeRect(rx, ry, rw, rh);
    });

    this.input.on("pointerup", (pointer: Phaser.Input.Pointer) => {
      if (this.isMobileInput && this.handleMobilePointerUp(pointer)) return;
      if (this.cameraDragging) {
  ...
  updateRtsCamera(delta: number) {
    ...
    let intentX = 0;
    let intentY = 0;
    const pointerInView = !!p && p.x >= 0 && p.x <= cam.width && p.y >= 0 && p.y <= cam.height;
    const pointerBusy = this.cameraDragging
      || !!this.selectionStart
      || this.isDraggingSelection
      || !!this.draggingBuildType
      || !!p?.isDown
      || (!!p && this.pointInRect(p.x, p.y, this.buildPanelBg));

    if (pointerInView && !pointerBusy && p) {
      if (p.x <= edge) intentX -= (edge - p.x) / edge;
      else if (p.x >= cam.width - edge) intentX += (p.x - (cam.width - edge)) / edge;
      if (p.y <= edge) intentY -= (edge - p.y) / edge;
      else if (p.y >= cam.height - edge) intentY += (p.y - (cam.height - edge)) / edge;
    }

    if (me?.isCoreAnchored) {
      if (this.cursors.left.isDown || this.wasd.A.isDown) intentX -= 1;
      if (this.cursors.right.isDown || this.wasd.D.isDown) intentX += 1;
      if (this.cursors.up.isDown || this.wasd.W.isDown) intentY -= 1;
      if (this.cursors.down.isDown || this.wasd.S.isDown) intentY += 1;
    }
    ...
    const targetSpeed = 560 / cam.zoom;
    const targetVX = intentX * targetSpeed;
    const targetVY = intentY * targetSpeed;
    const response = len > 0.0001 ? 14 : 11;
    const t = 1 - Math.exp(-response * dt);
    this.camVelX += (targetVX - this.camVelX) * t;
    this.camVelY += (targetVY - this.camVelY) * t;
    if (Math.abs(this.camVelX) < 0.5) this.camVelX = 0;
    if (Math.abs(this.camVelY) < 0.5) this.camVelY = 0;
    if (this.camVelX !== 0 || this.camVelY !== 0) {
      cam.scrollX += this.camVelX * dt;
      cam.scrollY += this.camVelY * dt;
      this.clampCameraToWorld();
    }
  }
  ...
  handleViewportResize(gameSize: Phaser.Structs.Size) {
    const width = Number(gameSize?.width || window.innerWidth || this.cameras.main.width);
    const height = Number(gameSize?.height || window.innerHeight || this.cameras.main.height);
  ...
  layoutMobileHud() {
    if (!this.isMobileInput || !this.mobileHudRootEl) return;
  ...
  adjustMobileZoom(delta: number) {
    const cam = this.cameras.main;
    const nextZoom = Phaser.Math.Clamp(cam.zoom + delta, 0.55, 2.2);
    if (Math.abs(nextZoom - cam.zoom) < 0.001) return;
    const centerWorldX = cam.worldView.centerX;
    const centerWorldY = cam.worldView.centerY;
    cam.setZoom(nextZoom);
    cam.centerOn(centerWorldX, centerWorldY);
    this.clampCameraToWorld();
    this.layoutBaseDefenseHud();
  }
  ...
  handleMobilePointerDown(pointer: Phaser.Input.Pointer) {
    ...
    if (this.getActiveTouchPointers().length >= 2) return true;
  ...
  handleMobilePointerMove(pointer: Phaser.Input.Pointer) {
    ...
    if (dist > 10) this.touchMoved = true;
  ...
  handleMobilePointerUp(pointer: Phaser.Input.Pointer) {
    ...
    if (this.mobileCommandMode === "select" && this.selectionStart) {
      if (this.isDraggingSelection) {
        ...
        const tapScreenDist = Math.hypot(pointer.x - this.mobileLastTapScreenX, pointer.y - this.mobileLastTapScreenY);
        ...
        if (clickedUnitId) {
          ...
          this.selectedStructureId = clickedStructureId;
          this.mobileLastTapAt = 0;
        } else if (isDoubleTap && this.selectedUnitIds.size > 0) {
          this.issueLocalUnitMoveCommand(world.x, world.y);
          this.selectedStructureId = null;
          this.mobileLastTapAt = 0;
        } else {
          ...
          this.mobileLastTapScreenX = pointer.x;
          this.mobileLastTapScreenY = pointer.y;
        }
      }
      this.selectionStart = null;
      this.isDraggingSelection = false;
  ...
  update(_time: number, delta: number) {
    ...
    if (me) {
      ...
      else if (phase === "build") this.statusText.setText(`Build phase: ${phaseLeft}s`);
      else this.statusText.setText("Battle phase");
      this.buildMenuText.setVisible(!this.isMobileInput);
    ...
    if (Phaser.Input.Keyboard.JustDown(this.keyEsc)) {
      ...
      this.selectedStructureId = null;
      this.stopBuildDrag();
      // Hide formation preview visualization (units keep moving)
      this.formationPreviewUntil = 0;
    }
    ...
    if (meEntity && !this.cameraHasInitialFocus) {
      this.cameras.main.centerOn(meEntity.x, meEntity.y);
      this.clampCameraToWorld();
...
const config: Phaser.Types.Core.GameConfig = {
  ...
  scene: [MenuScene, GameScene, BaseDefenseScene, BaseDefenseUiScene]
};

...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  updateTouchPinch() {
    ...
    const worldBefore = this.screenToWorldPoint(midX, midY);
    ...
    if (Math.hypot(midDx, midDy) > 0.6) {
      const worldPerSecond = ((1000 / dtMs) / Math.max(0.001, cam.zoom)) * TOUCH_GESTURE_SCROLL_BOOST;
    ...
    cam.setZoom(nextZoom);
    cam.preRender();
    const worldAfter = this.screenToWorldPoint(midX, midY);

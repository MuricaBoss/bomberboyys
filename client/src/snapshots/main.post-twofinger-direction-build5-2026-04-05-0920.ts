...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  drawMap(state: any) {
    const width = state.mapWidth;
    const height = state.mapHeight;
    this.add.rectangle(width * TILE_SIZE / 2, height * TILE_SIZE / 2, width * TILE_SIZE * 3, height * TILE_SIZE * 3, 0x1f5f1f).setDepth(-20);
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
  ...
  updateTouchPinch() {
    ...
    const nextZoom = Phaser.Math.Clamp(this.touchPinchStartZoom * (effectiveDist / this.touchPinchStartDist), 0.55, 2.2);
    ...
    if (Math.hypot(midDx, midDy) > 2) {
      const worldPerSecond = (1000 / dtMs) / Math.max(0.001, cam.zoom);
    ...
    cam.preRender();
    this.layoutBaseDefenseHud();
    if (Math.hypot(midDx, midDy) > 3 || Math.abs(nextZoom - this.touchPinchStartZoom) > 0.003) {

...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  adjustMobileZoom(delta: number) {
    const cam = this.cameras.main;
    const nextZoom = Phaser.Math.Clamp(cam.zoom + delta, MIN_CAMERA_ZOOM, MAX_CAMERA_ZOOM);
  ...
  updateTouchPinch() {
    ...
    const nextZoom = Phaser.Math.Clamp(this.touchPinchStartZoom * (effectiveDist / this.touchPinchStartDist), MIN_CAMERA_ZOOM, MAX_CAMERA_ZOOM);

...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  getCameraScrollForCenterWorld(centerX: number, centerY: number) {
    ...
    const minScrollX = viewW >= worldW ? worldW - viewW : 0;
    const maxScrollX = viewW >= worldW ? 0 : worldW - viewW;
    const minScrollY = viewH >= worldH ? worldH - viewH : 0;
    const maxScrollY = viewH >= worldH ? 0 : worldH - viewH;
    const scrollX = Phaser.Math.Clamp(centerX - viewW * 0.5, minScrollX, maxScrollX);
    const scrollY = Phaser.Math.Clamp(centerY - viewH * 0.5, minScrollY, maxScrollY);
  ...
  updateCameraClampBack(delta: number) {
    if (this.touchPinching || !this.room?.state) return;
    if (this.cameraClampBackX == null || this.cameraClampBackY == null) return;
    const cam = this.cameras.main;
    const dt = Math.max(0.001, Math.min(0.05, delta / 1000));
    const alpha = 1 - Math.exp(-10 * dt);
    cam.scrollX = Phaser.Math.Linear(cam.scrollX, this.cameraClampBackX, alpha);
    cam.scrollY = Phaser.Math.Linear(cam.scrollY, this.cameraClampBackY, alpha);
    this.syncCameraFocusToView();
    if (Math.abs(cam.scrollX - this.cameraClampBackX) <= 0.5 && Math.abs(cam.scrollY - this.cameraClampBackY) <= 0.5) {
      cam.scrollX = this.cameraClampBackX;
      cam.scrollY = this.cameraClampBackY;
      this.syncCameraFocusToView();
  ...
  updateTouchPinch() {
    ...
    cam.setZoom(nextZoom);
    const next = this.getClampedScrollForCurrentView(this.touchPinchLockedScrollX, this.touchPinchLockedScrollY);
    cam.scrollX = next.scrollX;
    cam.scrollY = next.scrollY;
    this.layoutBaseDefenseHud();
  ...
  endTouchPinch() {
    ...
    if (this.room?.state) {
      const cam = this.cameras.main;
      const next = this.getClampedScrollForCurrentView(cam.scrollX, cam.scrollY);
      this.cameraCenterTween?.remove();
      this.cameraCenterTween = null;
      if (Math.abs(next.scrollX - cam.scrollX) > 0.5 || Math.abs(next.scrollY - cam.scrollY) > 0.5) {
        this.cameraClampBackX = next.scrollX;
        this.cameraClampBackY = next.scrollY;
      } else {
        this.cameraClampBackX = null;
        this.cameraClampBackY = null;
        this.syncCameraFocusToView();
  ...
  handleMobilePointerUp(pointer: Phaser.Input.Pointer) {
    ...
    if (this.actionMode === "build" && !this.touchMoved) {
      ...
      if (me?.isAlive && (me.isCoreAnchored || me.devMode)) {
        const gx = Math.floor(tapWorld.x / TILE_SIZE);
        const gy = Math.floor(tapWorld.y / TILE_SIZE);
        if (this.canPlaceSelectedBuildAt(gx, gy)) {
          this.room.send("build_structure", { type: this.selectedBuild, gridX: gx, gridY: gy });
        } else {
  ...
  update(_time: number, delta: number) {
    ...
    this.updateCameraClampBack(delta);
    const nowMs = Date.now();

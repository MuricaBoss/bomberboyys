...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  edgeScrollHintsRootEl: HTMLDivElement | null = null;
  ...
  updateRtsCamera(delta: number) {
    ...
    const edge = Math.max(78, Math.floor(Math.min(cam.width, cam.height) * 0.09));
    let intentX = 0;
    let intentY = 0;
  ...
  destroyClientClockDom() {
    ...
    this.destroyEdgeScrollHintsDom();
  }
  ...
  createEdgeScrollHintsDom() {
    ...
    const makeEdge = (style: Partial<CSSStyleDeclaration>) => {
    ...
    root.appendChild(makeEdge({
      left: "0",
      top: "0",
      bottom: "0",
      width: "78px",
      background: "linear-gradient(90deg, rgba(120,220,255,0.16), rgba(120,220,255,0))",
    }));
    root.appendChild(makeEdge({
      right: "0",
      top: "0",
      bottom: "0",
      width: "78px",
      background: "linear-gradient(270deg, rgba(120,220,255,0.16), rgba(120,220,255,0))",
    }));
    root.appendChild(makeEdge({
      left: "0",
      right: "0",
      top: "0",
      height: "78px",
      background: "linear-gradient(180deg, rgba(120,220,255,0.14), rgba(120,220,255,0))",
    }));
    root.appendChild(makeEdge({
      left: "0",
      right: "0",
      bottom: "0",
      height: "88px",
      background: "linear-gradient(0deg, rgba(120,220,255,0.14), rgba(120,220,255,0))",
    }));

    this.getOverlayHostEl().appendChild(root);
    this.edgeScrollHintsRootEl = root;
  }

  destroyEdgeScrollHintsDom() {
    if (!this.edgeScrollHintsRootEl) return;
    this.edgeScrollHintsRootEl.remove();
    this.edgeScrollHintsRootEl = null;
  }
  ...
  updateTouchPinch() {
    ...
    const midY = (a.y + b.y) * 0.5;
    const cam = this.cameras.main;
    const nextZoom = Phaser.Math.Clamp(this.touchPinchStartZoom * (dist / this.touchPinchStartDist), 0.55, 2.2);
    cam.setZoom(nextZoom);

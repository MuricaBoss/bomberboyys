...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  screenToWorldPoint(screenX: number, screenY: number) {
    ...
    return {
      x: cam.worldView.x + screenX / Math.max(0.001, cam.zoom),
      y: cam.worldView.y + screenY / Math.max(0.001, cam.zoom),
    };
  }
  ...
  createClientClockDom() {
    ...
    root.style.background = "rgba(0,0,0,0.58)";
    root.style.color = "#ffffff";
    root.style.font = "700 24px Arial, sans-serif";

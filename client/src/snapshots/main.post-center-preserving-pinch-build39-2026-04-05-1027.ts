...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  updateTouchPinch() {
    ...
    cam.preRender();
    const centerX = cam.scrollX + cam.worldView.width * 0.5;
    const centerY = cam.scrollY + cam.worldView.height * 0.5;
  ...
  handleMobilePointerUp(pointer: Phaser.Input.Pointer) {
    ...
    const world = this.screenToWorldPoint(pointer.x, pointer.y);

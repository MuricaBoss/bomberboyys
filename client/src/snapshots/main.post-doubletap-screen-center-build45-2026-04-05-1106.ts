...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  handleMobilePointerUp(pointer: Phaser.Input.Pointer) {
    ...
    const world = this.screenToWorldPoint(releaseX, releaseY);
    ...
    if (this.mobileCommandMode === "select" && this.selectionStart) {
      if (this.isDraggingSelection) {
        ...
        const tapScreenDist = Math.hypot(releaseX - this.mobileLastTapScreenX, releaseY - this.mobileLastTapScreenY);
        ...
        if (shouldCenterAfterAction) {
          ...
          this.centerCameraOnScreenPoint(releaseX, releaseY, true);
        } else if (shortTap) {
          this.mobileLastTapAt = nowTapAt;
          this.mobileLastTapScreenX = releaseX;
          this.mobileLastTapScreenY = releaseY;
        } else {
          this.mobileLastTapAt = 0;
        }
      }

...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  updateTouchPinch() {
    ...
    const nextZoom = Phaser.Math.Clamp(cam.zoom * (1 + effectiveRatioDelta), this.getMinCameraZoom(), MAX_CAMERA_ZOOM);
    ...
    const prevMidX = this.touchPinchLastMidX;
    const prevMidY = this.touchPinchLastMidY;
    ...
    const prevMidWorld = this.screenToWorldPoint(prevMidX, prevMidY);
    if (midMove > 0.6) {
    ...
    cam.setZoom(nextZoom);
    cam.preRender();
    const currMidWorld = this.screenToWorldPoint(midX, midY);
    cam.scrollX += prevMidWorld.x - currMidWorld.x;
    cam.scrollY += prevMidWorld.y - currMidWorld.y;
    this.syncCameraAfterZoom();
    this.layoutBaseDefenseHud();

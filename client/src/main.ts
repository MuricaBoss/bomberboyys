...
const TILE_SIZE = 32;
const RTS_GROUND_TILE_SCALE = 0.25;
const RTS_BLOCK_TEXTURE_KEYS = ["block_1", "block_2", "block_3", "block_4"] as const;
const RTS_INTERIOR_WALL_VISUAL_SCALE = 1.5;
const RTS_BUILDING_TEXTURE_KEYS = {
...
const RTS_UI_TEXTURE_KEYS = {
  move_target_marker: "ui_move_target_marker",
} as const;
const RTS_TANK_TEXTURE_KEYS = {
  n: "tank_ready_n",
  ne: "tank_ready_ne",
  e: "tank_ready_e",
  se: "tank_ready_se",
...
const RTS_SOLDIER_SPRITESHEET_KEYS = {
  run: "soldier_run_sheet",
  shoot: "soldier_shoot_sheet",
} as const;
const RTS_SOLDIER_FRAME_SIZE = 32; // Optimized from 40 for Build 96 ultra-low perf
const RTS_SOLDIER_FRAME_COLS = 8;
const RTS_SOLDIER_ROW_BY_DIR = [2, 3, 4, 5, 6, 7, 0, 1] as const;
const RTS_SOLDIER_IDLE_FRAMES = [0, 0, 4, 4, 0, 7] as const;
const RTS_SOLDIER_PROJECTILE_RANGE = TILE_SIZE * 5.7; // Decreased 30% from 8.2
const RTS_TANK_PROJECTILE_RANGE = TILE_SIZE * 7.7;    // Decreased 30% from 11.0
const RTS_SOLDIER_PROJECTILE_INTERVAL_MS = 420;
const RTS_SOLDIER_PROJECTILE_SPEED = 420;
const RTS_SOLDIER_PROJECTILE_RADIUS = 2.4;
const RTS_TANK_PROJECTILE_SPEED = 300;
const RTS_TANK_PROJECTILE_RADIUS = 5.0;
const RTS_TANK_PROJECTILE_INTERVAL_MS = 1400;
const RTS_MOVE_CLICK_MARKER_LIFETIME_MS = 1200;
const RTS_TANK_DISPLAY_SIZE = 64;
const RTS_TANK_ORIGIN_Y = 406 / 420;
const RTS_SOLDIER_DISPLAY_SIZE = 25;
const RTS_SOLDIER_ORIGIN_Y = 0.84;
const RTS_PLAYER_SOLDIER_DISPLAY_SIZE = 28;
const RTS_PLAYER_CONSTRUCTOR_DISPLAY_SIZE = TILE_SIZE * 2.8;
const RTS_PLAYER_CONSTRUCTOR_ORIGIN_Y = 0.74;
const RTS_TANK_SELECTION_BOX_SIZE_SCALE = 0.68;
const RTS_TANK_SELECTION_CENTER_Y = 0.6;
const RTS_TANK_SELECTION_SIDE_Y_OFFSET = 3;
const RTS_TANK_HP_BOTTOM_OFFSET = 4;
const RTS_TANK_TRAIL_SEGMENT_LENGTH = 16;
const RTS_TANK_TRAIL_SEGMENT_WIDTH = 5;
const RTS_TANK_TRAIL_GAP = 12;
const RTS_TANK_TRAIL_BACK_OFFSET = 8;
const RTS_TANK_TRAIL_SPAWN_DISTANCE = 6;
const RTS_TANK_TRAIL_LIFETIME_MS = 2200;
const RTS_TANK_TRAIL_ALPHA = 0.58;
const RTS_IMAGE_SHADOW_ALPHA = 0.24;
const RTS_TILE_SHADOW_ALPHA = 0.24;
// Build 100: Removed unused telemetry constants
const WORLD_DEPTH_BASE = 10;
const WORLD_DEPTH_PER_PIXEL = 0.05;
const WORLD_DEPTH_TILE_OFFSET = 0.0;
const WORLD_DEPTH_TRAIL_OFFSET = 0.005;
const WORLD_DEPTH_RESOURCE_OFFSET = 0.01;
const WORLD_DEPTH_STRUCTURE_OFFSET = 0.02;
const WORLD_DEPTH_PLAYER_OFFSET = 0.03;
const WORLD_DEPTH_UNIT_OFFSET = 0.04;
const WORLD_DEPTH_PROJECTILE_OFFSET = 0.05;
const WORLD_DEPTH_SHADOW_GAP = 0.015;
const WORLD_DEPTH_SELECTION_OFFSET = -0.02;
const WORLD_DEPTH_LABEL_OFFSET = 0.35;
const WORLD_DEPTH_HP_OFFSET = 0.45;
...
class BaseDefenseScene_Advanced extends Phaser.Scene {
  ...
  getWallShadowSpec(gx: number, gy: number) {
    ...
    return {
      x: worldX,
      y: worldY + TILE_SIZE * 0.24,
      width: TILE_SIZE * 0.84,
    ...
    if (state.units?.forEach) {
      try {
        ...
        if (isTank || isSoldier) {
          ...
          const movingFast = Math.hypot(vx, vy) > 0.1;

          if (isLocalOwned && movingFast) {
            // Local prediction for immediate response when driving
            dir = this.angleToDir8(Math.atan2(vy, vx));
          } else if (typeof u.dir === "number") {
            // Priority: Use authoritative direction from server
            dir = u.dir;
          } else {
            // Secondary/Fallback Priority: Recent local shot
            ...
            if (lastShot && (Date.now() - lastShot.at) < 800) {
              ...
              // Remote unit moving fast but u.dir missing
              dir = this.angleToDir8(Math.atan2(vy, vx));
            } else if (!this.selectedUnitIds.has(id) || atkTargetId) {
              // Destination: Only rotate to face destination if NOT an idle selected unit
              const tx = Number(u.targetX ?? u.x);
              const ty = Number(u.targetY ?? u.y);
              const dx = tx - ux;
              const dy = ty - uy;
              if (Math.hypot(dx, dy) > 0.24) dir = this.angleToDir8(Math.atan2(dy, dx));
            }
          }
        ...
        if (isTank) {
          ...
          const moving = Math.hypot(Number(rs?.vx ?? 0), Number(rs?.vy ?? 0)) > 12
...
// Build 101: Scale manager (FIT) handles resizing automatically
